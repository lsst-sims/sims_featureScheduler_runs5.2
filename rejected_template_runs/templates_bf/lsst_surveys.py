...
def generate_blobs(
    ...
    safety_mask_params: dict | None = None,
    pair_pad: float = 5.0,
    n_obs_template: dict = {"u": 6, "g": 6, "r": 6, "i": 6, "z": 6, "y": 6},
    y1_template_weight: float = 5., 
) -> list[BlobSurvey]:
    """Generate surveys that take observations in blobs.
    ...
    for bandname, bandname2 in zip(band1s, band2s):
        ...
        # Templates for Y1
        if bandname2 is not None:
            if bandname in list(n_obs_template.keys()):
                bfs.append(
                    (
                        ...
                        y1_template_weight / 2,
                    )
                )
            if bandname2 in list(n_obs_template.keys()):
                bfs.append(
                    (
                        bf.NObsPerYearBasisFunction(
                            bandname=bandname2,
